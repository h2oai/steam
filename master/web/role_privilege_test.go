...
func TestOwnSharingOfRole(tt *testing.T) {
	t := newTest(tt)

	entityId, err := t.svc.CreateRole(t.su, "entity1", "description")
	t.nil(err)
	...
	err = t.svc.LinkRoleWithPermissions(t.su, role1Id, []int64{
		...
		permissionMap[data.ManageRole],
		permissionMap[data.ViewRole],
	})
	...
	err = t.svc.UpdateRole(user1, entityId, "entity2", "description")
	t.notnil(err)

	entityTypeMap := buildEntityTypeMap(t)

	// share as su
	err = t.svc.ShareEntity(t.su, data.CanEdit, group1Id, entityTypeMap[data.RoleEntity], entityId)
	t.nil(err)
	...
	entity, err := t.svc.GetRole(user1, entityId)
	t.nil(err)
	...
	_, err = t.svc.GetRole(user2, role2Id)
	t.notnil(err)

	// share as user1 to group2 -- should fail (user1 cannot view group2)
	err = t.svc.ShareEntity(user1, data.CanView, group2Id, entityTypeMap[data.RoleEntity], entityId)
	t.notnil(err)
	...
	// make user1 an owner
	err = t.svc.ShareEntity(t.su, data.Owns, group1Id, entityTypeMap[data.RoleEntity], entityId)
	t.nil(err)
	...
	_, err = t.svc.GetRole(user2, entityId)
	t.nil(err)
}

func TestEditSharingOfRole(tt *testing.T) {
	t := newTest(tt)
	...
	err = t.svc.UpdateRole(user, entityId, "entity2", "description")
	t.notnil(err)

	entityTypeMap := buildEntityTypeMap(t)

	// share as su
	err = t.svc.ShareEntity(t.su, data.CanEdit, groupId, entityTypeMap[data.RoleEntity], entityId)
	t.nil(err)
	...
	entity, err := t.svc.GetRole(user, entityId)
	t.nil(err)

	t.ok(entity.Name == "entity2", "entity name")
}

func TestViewSharingOfRole(tt *testing.T) {
	t := newTest(tt)
	...
	// share as su
	err = t.svc.ShareEntity(t.su, data.CanView, groupId, entityTypeMap[data.RoleEntity], entityId)
	t.nil(err)
