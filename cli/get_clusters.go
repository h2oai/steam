...
func returnType(id int64) string {
	if id == 1 {
		return "external"
	}
		...
		for i, cl := range cs {
			if !details {
				lines[i] = fmt.Sprintf("%s\t%d\t%s\t%s\t%s\t%s", cl.Name, cl.Id,
					cl.Address, cl.State, returnType(cl.TypeId), fmtAgo(cl.CreatedAt))
			} else {
				if (cl.State != data.StoppedState) || (cl.State != data.DisconnectedState) {
					...
					lines[i] = fmt.Sprintf("%s\t%d\t%s\t%s\t%d\t%s\t%s\t%s", cl.Name,
						...
						info.Status, returnType(cl.TypeId), fmtAgo(cl.CreatedAt))
				} else {
					lines[i] = fmt.Sprintf("%s\t%d\t%s\tNA\tNA\t%s\t%s\t%s", cl.Name,
						cl.Id, cl.Address, cl.State, returnType(cl.TypeId),
						fmtAgo(cl.CreatedAt))
				}
			}
		}
