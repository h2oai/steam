...
Commands:
    ...
    $ steam filter models ...
`
func filter(c *context) *cobra.Command {
  cmd := newCmd(c, filterHelp, nil)

  cmd.AddCommand(filterModels(c))
  return cmd
}


var filterModelsHelp = `
models [?]
Filter Models
Examples:

    No description available
    $ steam filter models --by-name \
        --project-id=? \
        --name-part=? \
        --offset=? \
        --limit=?
...
func filterModels(c *context) *cobra.Command {
  var byName bool // Switch for FilterModelsByName()
  ...
  cmd := newCmd(c, filterModelsHelp, func(c *context, args []string) {
    if byName { // FilterModelsByName
      
      // No description available
      models, err := c.remote.FilterModelsByName(
        projectId, // No description available
  ...
  cmd.Flags().BoolVar(&byName, "by-name", byName, "No description available")
